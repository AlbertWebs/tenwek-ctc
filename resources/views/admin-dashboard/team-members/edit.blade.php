...
@section('content')
    <div class="rounded-xl bg-white border border-gray-200 shadow-sm p-6 max-w-2xl">
        <form action="{{ route('admin-dashboard.team-members.update', $member) }}" method="post" class="space-y-5">
            @csrf
            @method('PUT')
            <div>
            ...
            <div>
                ...
                <textarea name="bio" id="bio" rows="4" class="w-full rounded-lg border border-gray-300 px-4 py-2 focus:ring-2 focus:ring-admin-teal focus:border-admin-teal">{{ old('bio', $member->bio) }}</textarea>
            </div>
            <div>
