            ...
            <tbody class="divide-y divide-gray-200">
                @forelse($sections as $s)
                    <tr class="hover:bg-admin-bg/50">
                        <td class="px-4 py-3 text-sm font-medium text-admin-dark sm:px-6">{{ $s->key }}</td>
                        <td class="px-4 py-3 text-sm text-admin-muted sm:px-6">{{ $s->title }}</td>
                ...
                @empty
                    <tr><td colspan="4" class="px-4 py-8 text-center text-admin-muted">No sections. <a href="{{ route('admin-dashboard.about.create') }}" class="text-admin-teal hover:underline">Add one</a>.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
