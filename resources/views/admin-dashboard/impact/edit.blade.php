...
@section('content')
    <div class="rounded-xl bg-admin-surface border border-gray-200 shadow-sm p-6 max-w-2xl">
        <form action="{{ route('admin-dashboard.impact.update', $story) }}" method="post" class="space-y-5">
            @csrf @method('PUT')
            ...
            <div><label class="block text-sm font-medium text-admin-dark mb-1">Story</label><textarea name="story" rows="6" class="w-full rounded-lg border border-gray-300 px-4 py-2 focus:ring-2 focus:ring-admin-teal">{{ old('story', $story->story) }}</textarea></div>
            <div><label class="block text-sm font-medium text-admin-dark mb-1">Image URL</label><input type="text" name="image" value="{{ old('image', $story->image) }}" class="w-full rounded-lg border border-gray-300 px-4 py-2 focus:ring-2 focus:ring-admin-teal"></div>
