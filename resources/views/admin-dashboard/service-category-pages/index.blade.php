@extends('admin-dashboard.layouts.app')
@section('title', 'Service area pages')
@section('header', 'Service area pages')

@section('content')
    <div class="max-w-4xl space-y-6">
        <div class="rounded-xl bg-admin-surface border border-gray-200 shadow-sm p-6">
            <p class="text-xs font-extrabold uppercase tracking-[0.22em] text-admin-muted">Public site</p>
            <h2 class="mt-2 text-xl font-semibold text-admin-dark">Category landing pages</h2>
            <p class="mt-1 text-sm text-admin-muted">
                Edit the long-form content on <code class="text-xs bg-admin-bg px-1 rounded">/services/cardiac-surgery</code>,
                <code class="text-xs bg-admin-bg px-1 rounded">/services/thoracic-surgery</code>, and
                <code class="text-xs bg-admin-bg px-1 rounded">/services/diagnostics</code>.
                Service cards below the article still come from <a href="{{ route('admin-dashboard.services.index') }}" class="text-admin-teal font-medium hover:underline">Services</a>.
            </p>
        </div>

        <div class="rounded-xl bg-admin-surface border border-gray-200 shadow-sm overflow-hidden">
            <ul class="divide-y divide-gray-100">
                @foreach($pages as $page)
                    <li class="flex flex-wrap items-center justify-between gap-4 p-5 hover:bg-admin-bg/60 transition-colors">
                        <div class="min-w-0">
                            <p class="font-semibold text-admin-dark">{{ $page->admin_label }}</p>
                            <p class="text-sm text-admin-muted mt-0.5 truncate">{{ $page->intro_heading }}</p>
                        </div>
                        <div class="flex items-center gap-2 shrink-0">
                            <a href="{{ route('services.category', ['serviceCategory' => $page->url_segment]) }}"
                               target="_blank" rel="noopener"
                               class="px-3 py-2 rounded-lg border border-gray-300 text-sm text-admin-dark hover:bg-white">
                                View live
                            </a>
                            <a href="{{ route('admin-dashboard.service-category-pages.edit', $page) }}"
                               class="px-3 py-2 rounded-lg bg-admin-teal text-white text-sm font-medium hover:bg-admin-teal-dark">
                                Edit
                            </a>
                        </div>
                    </li>
                @endforeach
            </ul>
        </div>
    </div>
@endsection
