            ...
            <tbody class="bg-white">
                @forelse($blocks as $b)
                    <tr class="hover:bg-admin-bg/50">
                        <td class="text-sm font-medium text-admin-dark">{{ $b->key }}</td>
                        <td class="text-sm text-admin-muted">{{ $b->title }}</td>
                        <td class="text-sm">{{ $b->is_visible ? 'Yes' : 'No' }}</td>
                ...
                @empty
                    <tr><td colspan="4" class="text-center text-admin-muted py-10">No blocks. <a href="{{ route('admin-dashboard.patient-info.create') }}" class="text-admin-teal hover:underline">Add one</a>.</td></tr>
                @endforelse
            </tbody>
        </table>
        </div>
