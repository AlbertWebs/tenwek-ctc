...
@section('header', 'Edit milestone')

@section('content')
    <div class="rounded-xl bg-white border border-gray-200 shadow-sm p-6 max-w-2xl">
        <form action="{{ route('admin-dashboard.history-milestones.update', $milestone) }}" method="post" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="number" name="year" id="year" value="{{ old('year', $milestone->year) }}" min="1900" max="2100"
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="5"
                          class="w-full rounded-lg border border-gray-300 px-4 py-2 focus:ring-2 focus:ring-admin-teal focus:border-admin-teal">{{ old('description', $milestone->description) }}</textarea>
            </div>

            <div class="flex items-center">
                <input type="checkbox" name="is_visible" id="is_visible" value="1" {{ old('is_visible', $milestone->is_visible) ? 'checked' : '' }}
