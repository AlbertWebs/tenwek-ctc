...
<section
    class="relative bg-ctc-blue text-white overflow-hidden flex items-center justify-center min-h-[92svh] sm:min-h-[100svh]"
    data-ctc-hero
>
    @if($useCarousel)
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10 flex justify-center">
        <div class="relative max-w-3xl w-full px-5 sm:px-10 py-8 sm:py-12">
            ...
            <div class="relative z-10 text-center p-2 sm:p-3">
                <h1 id="ctc-hero-title" class="font-headline font-extrabold text-white tracking-[-0.035em] leading-[1.05] mb-4 sm:mb-5 drop-shadow-[0_18px_45px_rgba(0,0,0,0.45)]" style="font-size:clamp(2.05rem,5vw,4rem);">
                    {{ $title }}
                </h1>
            @if($subtitle)
                <p id="ctc-hero-subtitle" class="font-headline text-[0.62rem] sm:text-[0.65rem] font-bold uppercase tracking-[0.22em] mb-4 sm:mb-5">
                    <span class="inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-2 border border-white/15 shadow-[0_18px_45px_rgba(0,0,0,0.25)]">
            ...
            @endif
            @if($description)
                <p id="ctc-hero-description" class="font-sans text-white/80 font-light max-w-lg mx-auto mb-7 sm:mb-9 text-[0.92rem] sm:text-[0.95rem] leading-[1.7] sm:leading-[1.75] drop-shadow-[0_18px_45px_rgba(0,0,0,0.35)]">{{ $description }}</p>
            @endif
            @if(count($buttons) > 0)
                @php
    ...
    @if(!($useCarousel && $slides->count() > 0))
        <button
            type="button"
            class="absolute bottom-6 left-1/2 z-20 flex -translate-x-1/2 cursor-pointer flex-col items-center gap-1 rounded-lg border border-transparent bg-transparent p-1 text-white/70 transition-colors hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-ctc-accent focus-visible:ring-offset-2 focus-visible:ring-offset-ctc-blue/40 motion-reduce:hidden"
